...
from sklearn.metrics.pairwise import cosine_similarity  # to compute semantic similarities between embeddings
# Embedding model for semantic cohesion analysis across intents
...
def analyze_intent_json(json_path):
    # Load intent-to-utterances mapping from JSON
    with open(json_path, encoding='utf-8') as f:
        data = json.load(f)
    # Tokenizer splits text into alphanumeric tokens
    tokenizer = RegexpTokenizer(r'\w+')
    # Percentile thresholds for length summaries
    ...
    for intent, utterances in data.items():
        ...
        sentence_lengths = [len(tokenizer.tokenize(t.lower())) for t in texts]
        # Flatten all word tokens across utterances
        all_words = [w for t in texts for w in tokenizer.tokenize(t.lower())]
        ...
        syllable_counts = [count_syllables(w) for w in all_words]
        ...
        stats = {
            ...
            'sentence_length_percentiles': {str(p): float(np.percentile(sentence_lengths, p)) for p in percentiles} if sentence_lengths else {},
            ...
            'word_length_percentiles': {str(p): float(np.percentile(word_lengths, p)) for p in percentiles} if word_lengths else {},
...
if __name__ == '__main__':
    ...
    train_stats, train_dist = analyze_intent_json(base / 'utterances.json')
    test_stats, test_dist = analyze_intent_json(base / 'tests.json')
    # Save separate stats
    ...
    # Divergence between train and test
    divergence = {}
    for intent in set(train_dist) | set(test_dist):
        # Compute Earth Mover's Distance (EMD) for sentence and word lengths
        # Sentence lengths
    ...
    # Combined boxplots: visualize sentence/word length distributions per intent
    ...
    plt.tight_layout()
    plt.savefig(str(base / 'sentence_length_boxplot.png'))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(str(base / 'word_length_boxplot.png'))
    plt.close()
    print('Saved sentence/word length boxplots')
