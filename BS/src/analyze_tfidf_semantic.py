#!/usr/bin/env python3
"""
analyze_tfidf_semantic.py

Semantic analysis using TF-IDF vectorization with character n-grams to approximate
the behavior of sentence-transformers for semantic cohesion analysis.
"""
import numpy as np
import pandas as pd
...
class SemanticAnalyzer:
    """Analyzes semantic cohesion using TF-IDF with character n-grams."""
    
    def __init__(self):
        """Initialize with TF-IDF vectorizer configured for semantic analysis."""
        # Use character n-grams for better semantic representation in Spanish
...
def analyze_datasets(test_dir, train_dir, results_dir):
    ...
    pd.DataFrame(test_cohesion).to_csv(
        results_dir / 'test_suites_intra_semantic_cohesion.csv', index=False)
    pd.DataFrame(train_cohesion).to_csv(
        results_dir / 'utterances_intra_semantic_cohesion.csv', index=False)
    ...
    for fname in sorted(common):
        ...
        if v1 is not None and v2 is not None:
            ...
            sim = None
        cross_sim.append({'file': fname, 'cross_similarity': sim})
    
    pd.DataFrame(cross_sim).to_csv(
        results_dir / 'cross_semantic_similarity.csv', index=False)
    ...
    pd.DataFrame(test_pairs).to_csv(
        results_dir / 'test_suites_intra_semantic_similarity.csv', index=False)
    ...
    pd.DataFrame(train_pairs).to_csv(
        results_dir / 'utterances_intra_semantic_similarity.csv', index=False)
