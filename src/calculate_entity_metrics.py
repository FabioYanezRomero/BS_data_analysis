...
ENTITY_RESULTS_DIR = ROOT_DIR / 'Results' / 'Entities'
...
def process_all_batch_results():
    ...
    for results_file in BATCH_RESULTS_DIR.glob('*_results.csv'):
        ...
        # Save to CSV files
        entity_metrics_df.to_csv(ENTITY_RESULTS_DIR / f'{model_name}_entity_metrics.csv', index=False)

        # For entity identification file, group by entity type for better readability
        entity_identification_df.to_csv(ENTITY_RESULTS_DIR / f'{model_name}_entity_identification.csv', index=False)

        
        # Save entity methods without the Total rows for better readability
        entity_method_df.to_csv(ENTITY_RESULTS_DIR / f'{model_name}_entity_methods.csv', index=False)
        
        # Save engine summary to a separate file
        entity_engine_summary_df.to_csv(ENTITY_RESULTS_DIR / f'{model_name}_entity_engine_summary.csv', index=False)
        
        entity_success_df.to_csv(ENTITY_RESULTS_DIR / f'{model_name}_entity_success_koreai.csv', index=False)
        
        print(f"Generated entity metrics for {model_name} at {ENTITY_RESULTS_DIR}")
